...
const artifactFilename = {
  ...
  /* PPT Engine custom artifacts — rendered by dedicated React components,
   * not Sandpack. The filenames are placeholders that satisfy the type
   * system but are never used for file routing. */
  'application/vnd.ppt.outline': 'ppt-outline.json',
  'application/vnd.ppt.progress': 'ppt-progress.json',
  'application/vnd.ppt.preview': 'ppt-preview.json',
...
> = {
  ...
  /* PPT Engine artifacts bypass Sandpack entirely — undefined template
   * signals to ArtifactTabs that custom rendering is needed. We still
   * use 'static' here for type-system satisfaction; the actual rendering
   * is handled by the isPPTArtifact branch in ArtifactTabs.tsx. */
  'application/vnd.ppt.outline': 'static',
  'application/vnd.ppt.progress': 'static',
  'application/vnd.ppt.preview': 'static',
  default: 'static',
  // 'css': 'css',
  // 'javascript': 'js',
  // 'typescript': 'ts',
...
> = {
  ...
  /* PPT Engine artifacts — no Sandpack deps needed, components render natively. */
  'application/vnd.ppt.outline': {},
  'application/vnd.ppt.progress': {},
  'application/vnd.ppt.preview': {},
...
export const TOOL_ARTIFACT_TYPES = {
  ...
  /* PPT Engine artifact types — each maps to a dedicated React component
   * in ArtifactTabs. Content is a JSON payload parsed by the component. */
  PPT_OUTLINE: 'application/vnd.ppt.outline',
  PPT_PROGRESS: 'application/vnd.ppt.progress',
  PPT_PREVIEW: 'application/vnd.ppt.preview',
} as const;
...
/**
 * PPT Engine artifact types — rendered by dedicated React components
 * instead of the Sandpack sandbox. The content payload is JSON.
 */
const PPT_ARTIFACT_TYPES: ReadonlySet<ToolArtifactType> = new Set([
  TOOL_ARTIFACT_TYPES.PPT_OUTLINE,
  TOOL_ARTIFACT_TYPES.PPT_PROGRESS,
  TOOL_ARTIFACT_TYPES.PPT_PREVIEW,
]);

export function isPPTArtifact(type: string | null | undefined): boolean {
  if (type == null) {
    return false;
  }
  return PPT_ARTIFACT_TYPES.has(type as ToolArtifactType);
}
