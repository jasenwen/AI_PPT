...
export default function useArtifactProps({ artifact }: { artifact: Artifact }) {
  ...
  const [fileKey, files] = useMemo(() => {
    ...
    /* PPT Engine custom artifacts — content is a JSON payload that the
     * dedicated React components in ArtifactTabs parse directly. The
     * files/fileKey are placeholders; rendering bypasses Sandpack. */
    if (
      type === TOOL_ARTIFACT_TYPES.PPT_OUTLINE ||
      type === TOOL_ARTIFACT_TYPES.PPT_PROGRESS ||
      type === TOOL_ARTIFACT_TYPES.PPT_PREVIEW
    ) {
      return ['ppt-data.json', { 'ppt-data.json': artifact.content ?? '{}' }];
    }
